...
check: $(ISO) $(OVMF_VARS)
	...
	grep -q "phase8: pipe begin" $(LOG)
	grep -q "phase8-pipe" $(LOG)
	grep -q "phase8: pipe end" $(LOG)
...
check-disk: | $(BUILD_DIR)
	...
	grep -q "phase8: pipe begin" $(DISK_LOG)
	grep -q "phase8-pipe" $(DISK_LOG)
	grep -q "phase8: pipe end" $(DISK_LOG)
	grep -q "phase8: tty /dev/console" $(DISK_LOG)
...
check-disk-recovery: | $(BUILD_DIR)
	...
	grep -q "phase8: pipe begin" $(DISK_RECOVERY_LOG)
	grep -q "phase8-recovery" $(DISK_RECOVERY_LOG)
	grep -q "phase8: pipe end" $(DISK_RECOVERY_LOG)
...
check-disk-reinstall: | $(BUILD_DIR)
	...
	grep -q "phase8-reinstall" $(DISK_REINSTALL_LOG2)
